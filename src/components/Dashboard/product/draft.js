...
const ProductForm = (props) => {
    ...
    const [privateDesignPng, setPrivateDesignPng] = useState(null); // New state for single PNG image
    ...
    // New handler for privateDesignPng drop
    const handlePrivateDesignDrop = (newFile) => {
        if (newFile.length > 1) {
            setErrorMessage('Please select only one image.');
        } else {
            setPrivateDesignPng(newFile[0]);
            formik.setFieldValue('privateDesignPng', newFile[0]);
        }
    };

    // New handler to remove privateDesignPng
    ...
    const onSubmitHandler = async (
        ...
        try {
            ...
            if (values.privateDesignPng) {
                formData.append('privateDesignPng', values.privateDesignPng); // Adding the private design PNG
            }
                    ...
                    <Card>
                        <CardContent>
                            <Grid container spacing={4}>
                                <Grid xs={12} md={12}>
                                    <FileDropzone
                                        caption="Supported formats: JPEG, PNG"
                                        accept={{ "image/*": [] }}
                                        files={files}
                                        ...
                                        id={initialData?._id}
                                        title={"Drag & drop T-shirt pictures (Max 5)"}
                                    />
                                </Grid>
                                <Grid xs={12} md={12}>
                                    <FileDropzone
                                        caption="Supported format: PNG"
                                        accept={{ "image/png": [] }}
                                        files={privateDesignPng ? [privateDesignPng] : []}
                                        onDrop={handlePrivateDesignDrop}
                                        onRemove={handleRemovePrivateDesign}
                                        error={touched.privateDesignPng && errors.privateDesignPng}
                                        id={initialData?._id}
                                        title={"Drag & drop private design (1 PNG)"}
